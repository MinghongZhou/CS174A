...
triangle.prototype.populate = function( recipient, points_transform)
{
    ...
    recipient.vertices.push(vec3(1, 0, 0), vec3(0, -1, 0),vec3(0, 1, 0));
    recipient.vertices.push(vec3(-1, 0, 0), vec3(0, -1, 0), vec3(0, 1, 0));
    recipient.vertices.push(vec3(1, 0, 0), vec3(0, -1, 0), vec3(0, 0, 3));
    recipient.vertices.push(vec3(0, 1, 0), vec3(1, 0, 0), vec3(0, 0, 3));
    recipient.vertices.push(vec3(0, -1, 0), vec3(-1, 0, 0), vec3(0, 0, 3));
    recipient.vertices.push(vec3(-1, 0, 0), vec3(0, 1, 0), vec3(0, 0, 3));
    
    recipient.normals.push( vec3(0,0,1), vec3(0,0,1), vec3(0,0,1) );
    recipient.normals.push( vec3(0,0,1), vec3(0,0,1), vec3(0,0,1) );
    recipient.normals.push( vec3(-3,3,-1), vec3(-3,3,-1), vec3(-3,3,-1) );
    recipient.normals.push( vec3(-3,-3,-1), vec3(-3,-3,-1), vec3(-3,-3,-1) );
    recipient.normals.push( vec3(3,3,-1), vec3(3,3,-1), vec3(3,3,-1) );
    recipient.normals.push( vec3(-3,3,1), vec3(-3,3,1), vec3(-3,3,1) );
    ...
    for( var i = index_offset; i < recipient.indices.length; i++ ) recipient.indices[i] += offset;
    for( var i = offset; i < recipient.vertices.length; i++ )
        recipient.vertices[i] = vec3( mult_vec( points_transform, vec4( recipient.vertices[ i ], 1 ) ) );
}
...
function triangle_p() // Combine a tube and two flattened triangle fans to make a solid cylinder
{
    ...
    this.populate = ( function (self)
                     ...
                     object_transform = mult( object_transform, scale( 1, 1, .7));

                     self.m_triangle.populate( self, object_transform );
                     ...
                     //self.m_triangle.populate( self, object_transform, counter );
